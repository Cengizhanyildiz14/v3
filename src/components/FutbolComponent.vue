<template>
  <div class="relative flex flex-col px-16 bg-no-repeat bg-futbol-pattern bg-full bg-center-center">
    <div class="relative p-6 text-2xl font-extrabold text-center text-gray-800 bg-blue-200 rounded-lg shadow-md">
      {{ activityName }}
      <button @click="toggleMusic" class="absolute px-4 py-2 transition-transform duration-300 transform -translate-y-1/2 rounded top-1/2 right-4 hover:scale-125" v-html="isPlaying ? '<i class=\'fa-solid fa-pause\'></i>' : '<i class=\'fa-solid fa-play\'></i>'">
      </button>
    </div>
    <div class="flex items-center justify-between p-2 text-center bg-gray-200">
      ...
      <div class="flex flex-grow justify-evenly">
        <div v-for="day in days" :key="day.date"
          :class="['flex flex-col items-center justify-center border rounded-lg shadow-md p-4 mx-1 cursor-pointer transition duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-105', { 'bg-blue-200': selectedDay === day.date, 'bg-white': selectedDay !== day.date }]"
    ...
    <div class="mt-4">
      <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3068.718446323613!2d37.04572777597686!3d39.72350977155952!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x4079556931024473%3A0xa39644ba82d209f7!2zVcSeVVIgw4fEsEZUw4fEsCBTUE9SIEtPTVBMRUtTxLA!5e0!3m2!1str!2str!4v1717086437292!5m2!1str!2str"
        class="w-full h-60" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
    </div>
    <audio ref="audioPlayer" src="/music/saniser-live-sessions-beni-bana-yar-etmezler.mp3" loop></audio>
  </div>
</template>
  ...
  methods: {
    ...
    confirmAppointment() {
      ...
      if (selectedAppointmentDate <= now) {
        this.appointmentSuccess = false; // Bu satırı ekleyin
        this.showModalWithContent("Uyarı", "Geçmiş bir zaman dilimi için randevu oluşturamazsınız.");
        return;
      }

      if (!this.selectedDay || !this.selectedTime || selectedAppointmentDate < now) {
